...
| ----------------------- | ----------------- | --------------------------------------------------------- | ------------ |
| RuleGate NuGet packages | `0.8.0-preview.2` | .NET 8 (`net8.0`), .NET 9 (`net9.0`), .NET 10 (`net10.0`) | NuGet        |
...
```bash
dotnet add package Fotbiler.RuleGate.AspNetCore --version 0.8.0-preview.2
...
```bash
dotnet add package Fotbiler.RuleGate.Abstractions --version 0.8.0-preview.2
```
...
```bash
dotnet add package Fotbiler.RuleGate.Keycloak --version 0.8.0-preview.2
```
...
The `0.8.0-preview.2` tool evaluates deterministic authorization requests
against a manifest without starting the host application:
...
[`0.8.0-preview.2`](https://github.com/fotbiler-lab/rulegate/releases/tag/v0.8.0-preview.2).
It adds deterministic policy testing with explicit request fixtures, fixed
evaluation times, outcome and failure-code expectations, filtering, and text
or JSON reports.

All NuGet packages are `0.8.0-preview.2`. The independently versioned Angular
